          ...
          {stepId === 'services' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button type="button" className="vendor-cta rounded-xl px-4 py-2 text-xs font-semibold">Add service</button>
              </div>
              {vendorState.services.length === 0 ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {vendorState.services.map((service) => (
                    <div key={service.id} className="rounded-2xl border border-slate-200 p-4 bg-white shadow-sm transition-hover hover:shadow-md">
                      <div className="flex items-center justify-between">
                      ...
                      <div className="mt-3 text-xs text-slate-400">Packages: {service.packages.map((pkg) => pkg.name).join(', ')}</div>
                    </div>
                  ))}
                </div>
              )}
